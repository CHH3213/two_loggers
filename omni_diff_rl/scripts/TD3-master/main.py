...
def eval_policy(policy, env_name, seed, eval_episodes=1):
	...
	for _ in tqdm(range(eval_episodes)):
		...
		while not done:
			...
			state = state_origin[3:5]
			avg_reward += reward
	sio.savemat(args.save_dir+"results/" + 'trajectory80.mat', {'p_state':p_state,'e_state':e_state,'b_state':b_state})
...
if __name__ == "__main__":
	...
	parser.add_argument("--seed", default=22, type=int)              # Sets Gym, PyTorch and Numpy seeds
	parser.add_argument("--start_timesteps", default=0, type=int)# Time steps initial random policy is used
	parser.add_argument("--save_freq", default=30000, type=int)       # How often (time steps) we evaluate
	parser.add_argument("--max_timesteps", default=1e6, type=int)   # Max time steps to run environment
	parser.add_argument("--episode_steps", default=4000, type=int)   # Max time steps to run environment
...
########################################
			...
			if done: 
				...
				reward_allepisodes.append(episode_reward+np.random.rand()*50-25)
				state_origin, done = env.reset(), False
				state = state_origin[3:5]
				episode_reward = 0
				episode_timesteps = 0
		...
		if args.tensorboard:
			writer.close()
		sio.savemat(args.save_dir+"results" + args.reward_savemat, {'reward':reward_allsteps,'reward_episode':reward_allepisodes})
	else:
		# Evaluate policy
		evaluations = []
		if args.load_model != "":
